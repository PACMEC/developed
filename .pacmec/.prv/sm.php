...
namespace SolveMedia
{
  ...
  */
  ...
  {
   if ($pubkey == null || $pubkey == '') { die ("To use solvemedia you must get an API key from <a href='" . ADCOPY_SIGNUP . "'>" . ADCOPY_SIGNUP . "</a>"); }
  ...
  * This is called from the browser, and the resulting solvemedia HTML widget
  * is embedded within the HTML form it was called from.
  * @param string $div_id id div content
  * @param boolean $use_ssl Should the request be made over ssl? (optional, default is false)
  * @return string - The HTML to be embedded in the user's form.
  */
  function solvemedia_get_form_custom ($use_ssl=null, $div_id = null, $theme='custom', $div_id_custom='my_id')
  {
    $div_id = $div_id==null ? false : $div_id;
    $div_id_custom = $div_id_custom==null ? false : $div_id_custom;
    if ($use_ssl) { $server = ADCOPY_API_SECURE_SERVER; } else { $server = ADCOPY_API_SERVER; }
    $r = "";
    $id_tmp = ($div_id==false) ? \randString(11) : $div_id;
    if($div_id==false) $r .= "<div id=\"{$id_tmp}\"></div>";
    $r .= "<script type=\"text/javascript\">";
    $r .= "window.addEventListener('load', function(){";
      if($div_id==false) $r .= "ACPuzzle.create('".infosite('solvemedia_k_c')."', '{$id_tmp}', { lang: '".$GLOBALS['PACMEC']['lang']."', theme: '{$theme}', multi: true });";
      else $r .= "ACPuzzle.create('".infosite('solvemedia_k_c')."', '{$id_tmp}', { id: '{$div_id_custom}', lang: '".$GLOBALS['PACMEC']['lang']."', theme: '{$theme}', multi: true });";
    $r .= "})";
    $r .= "</script>";
    return $r;
    //    ACPuzzle.create('k4VkjuIqIF0XaBrZwNLOFc-lLi8FAR-c', '', { multi: true, id: 'ItQr1blf88v', theme: 'black', lang: 'es' });
  }
  ...
  */
  ...
  function check_answer($adcopy_challenge, $adcopy_response)
  {
    if(isset($adcopy_challenge) && !empty($adcopy_challenge) && isset($adcopy_response) && !empty($adcopy_response)){
      $solvemedia_response = solvemedia_check_answer(infosite('solvemedia_k_v'),
      					\getIpRemote(),
      					$adcopy_challenge,
      					$adcopy_response,
      					\infosite('solvemedia_k_h'));
      if (!$solvemedia_response->is_valid) {
      	return (\str_replace([' ', '+', '%20', '++'], ['_'], $solvemedia_response->error));
      }
      else {
      	return ('solvemedia_success');
      }
    } else {
